...
TOKEN_FILE_NAME = 'corePyApp/token.pickle'
...
def get_authenticated_service():
  ...
  if not creds or not creds.valid:
    if creds and creds.expired and creds.refresh_token:
      ...
      creds = flow.run_local_server(port=3003, authorization_prompt_message="Waiting for authorization. See message above.", redirect_uri_trailing_slash=False)
...
def get_current_user(config):
  ...
  if config == None:
    ...
    if config['your_channel_id'] == channelID:
      ...
      print("Please check the config file. Press Enter to Exit...")
      sys.exit()
  else:
