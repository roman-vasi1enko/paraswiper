...
def load_config_file(configVersion=None, forceDefault=False, skipConfigChoice=False, configFileName="SpamPurgeConfig.ini", configFolder="configs"):
  ...
  def default_config_path(relative_path):
    ...
    return os.path.join(os.path.abspath("worker/corePyApp/assets"), relative_path) # If running as script, specifies resource folder as /assets
...
def parse_comment_list(config, recovery=False, removal=False, returnFileName=False):
  ...
  if manuallyEnter == True:
    ...
    data = str(print("Paste the list here, then hit Enter: "))
    if str(data).lower() == "x":
      return "MainMenu", None
    print("\n")
  ...
  if notValidCount == 0:
    ...
    print(f"Press Enter to try {actionNoun} anyway...\n")
    ...
    if forceOverwrite == False:
      ...
      if False == True:
        ...
        while confirm == False:
          ...
          newFileName = "dictToPickleFile_% s" % (random.randint(1, 1000)) + ".save"
