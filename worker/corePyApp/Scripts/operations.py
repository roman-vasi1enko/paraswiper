...
def get_comments(current, filtersDict, miscData, config, allVideoCommentsDict, scanVideoID=None, nextPageToken=None, videosToScan=None):  # None are set as default if no parameters passed into function
  ...
  fieldsToFetch = "nextPageToken,items/snippet/topLevelComment/id,items/replies/comments,items/replies/comments/snippet/authorProfileImageUrl,items/snippet/totalReplyCount,items/snippet/topLevelComment/snippet/videoId,items/snippet/topLevelComment/snippet/authorChannelId/value,items/snippet/topLevelComment/snippet/authorDisplayName,items/snippet/topLevelComment/snippet/textDisplay,items/snippet/topLevelComment/snippet/publishedAt,items/snippet/topLevelComment/snippet/authorProfileImageUrl"
  ...
  for item in results["items"]:
    ...
    try:
      authorProfilePic = comment["snippet"]["authorProfileImageUrl"]
    except KeyError:
      authorProfilePic = "[NO URL]"
...
def get_replies(current, filtersDict, miscData, config, parent_id, videoID, parentAuthorChannelID, videosToScan, allVideoCommentsDict, parentCommentDict=None, repliesList=None):
  ...
  if repliesList == None:
    fieldsToFetch = "nextPageToken,items/snippet/authorChannelId/value,items/id,items/snippet/authorDisplayName,items/snippet/textDisplay,items/snippet/publishedAt,items/snippet/authorProfileImageUrl"
    replies = []
    replyPageToken = None
...
def add_spam(current, config, miscData, currentCommentDict, videoID, matchReason="Filter Match"):
  ...
  dictToUse[commentID] = {'text':commentText, 'textUnsanitized':commentTextRaw, 'authorName':authorChannelName, 'authorID':authorChannelID, 'videoID':videoID, 'matchReason':matchReason, 'originalCommentID':originalCommentID, 'timestamp':timestamp, 'authorProfilePic':currentCommentDict['authorProfilePic']}
...
def check_against_filter(current, filtersDict, miscData, config, currentCommentDict, videoID, allThreadAuthorNames=None):
  ...
  #   authorChannelName = print("Channel Name: ")
  #   commentText = print("Comment Text: ")
  #   authorChannelID = "x"
  ...
  if auth.CURRENTUSER.id != authorChannelID and miscData.channelOwnerID != authorChannelID and authorChannelID not in miscData.resources['Whitelist']['WhitelistContents']:
    ...
    ## Also Check if reply author ID is same as parent comment author ID, if so, ignore (to account for users who reply to spammers)
      ...
      # if debugSingleComment == True: 
      #   if print("Sensitive True/False: ").lower() == 'true': sensitive = True
...
def delete_found_comments(commentsList, banChoice, deletionMode, recoveryMode=False, skipCheck = False):
  ...
  def setStatus(commentIDs, failedComments): #Does the actual deletion
    if deletionMode == "reportSpam":
      ...
      print("Invalid deletion mode. This is definitely a bug.")
...
def exclude_authors(current, config, miscData, excludedCommentsDict, authorsToExcludeSet, commentIDExcludeSet, displayString, inputtedString, logInfo=None, only=False):
  ...
  for comment in current.matchedCommentsDict.keys():
    if comment in commentIDExcludeSet:
      ...
      # print(f"You should {F.YELLOW}DEFINITELY{S.R} report this bug.")
